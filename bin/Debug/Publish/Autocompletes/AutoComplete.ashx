...
{    
    ...
    public String AutoCompleteSearchStyle(string searchstyletext)
    {
        using (SqlConnection conn = new SqlConnection())
        {
            ...
            {
                cmd.CommandText = " SELECT DISTINCT [StyleNo]+'#'+[Buy] As StyleNo, [OrderNo] FROM [MerchandisingHO].[dbo].[Orders] "
                  + "WHERE [StyleNo] LIKE '%' + @SearchText + '%'  ORDER BY [OrderNo] DESC ";  //AND [Status] = 1
                cmd.Parameters.AddWithValue("@SearchText", searchstyletext.Trim());
                cmd.Connection = conn;
